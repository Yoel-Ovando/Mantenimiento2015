...
$.ajax({
      type: "post",
      url: 'http://localhost:8080/LAGY/solicitarListarCiudadesBuscar.do?ciudadBuscar='+valor,
...
error: function() {
	...
	$( "#nombres" ).click(function() {		
		$.post("http://localhost:8080/LAGY/procesarListadoGenteOrd.do", { ordenar: "nombres", tipo: valorNombres })
		.done(function(data) {		  	
		  	if(valorNombres==0) {
				$('#update').html(data);
		  		valorNombres=1; 
	...
	$( "#apellidos" ).click(function() {
		$.post("http://localhost:8080/LAGY/procesarListadoGenteOrd.do", { ordenar: "apellidos", tipo: valorApellidos })
		.done(function(data) {
		  	$('#update').html(data);
		  	if(valorApellidos==0) {
		  		valorApellidos=1;
	...
	$( "#direccion" ).click(function() {
		$.post("http://localhost:8080/LAGY/procesarListadoGenteOrd.do", { ordenar: "direccion", tipo: valorDireccion })
		.done(function(data) {
			console.log(data);
		  	$('#update').html(data);
		  	if(valorDireccion==0) {
	...
	$( "#telefono" ).click(function() {
		$.post("http://localhost:8080/LAGY/procesarListadoGenteOrd.do", { ordenar: "telefono", tipo: valorTelefono })
		.done(function(data) {
		  	$('#update').html(data);
		  	if(valorTelefono==0) {
		  		valorTelefono=1;
	...
	$( "#ciudad" ).click(function() {
		$.post("http://localhost:8080/LAGY/procesarListadoGenteOrd.do", { ordenar: "ciudad", tipo: valorCiudad })
		.done(function(data) {
		  	$('#update').html(data);
		  	if(valorCiudad==0) {
		  		valorCiudad=1;
...
function addTest() {
		...
		$.post("http://localhost:8080/LAGY/procesarRegistroTest.do", { nombre: nombre, descripcion: descripcion }).done(function(data) {
		  	alert("Test: "+nombre+" agregado");
		}).fail(function(e) {
		    alert(console.log(e));
	  	});
	}
